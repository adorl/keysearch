/*
 * sha256_avx2.c — SHA256 8-way AVX2 parallel compression function
 *
 * Processes 8 independent message blocks (each 64 bytes) simultaneously,
 * using AVX2 256-bit registers to pack 8 uint32_t values into one __m256i,
 * advancing 8 lanes of state with each SIMD instruction.
 *
 * Interface:
...
/* One SHA256 round (vectorized): advances 8 lanes of state simultaneously */
...
/* Load the i-th uint32_t (big-endian) from each of 8 blocks into a __m256i */
static inline __m256i load_be32_8way(const uint8_t *const blocks[8], int i)
{
    /* Offset of the i-th word in each block = i*4 */
    int off = i * 4;
...
/* Write the 8 lanes of a __m256i back to the i-th element of each of 8 states */
...
/*
 * Perform one SHA256 compression on 8 independent (state, block) pairs simultaneously
 */
__attribute__((target("avx2")))
void sha256_compress_avx2(uint32_t *states[8], const uint8_t *blocks[8])
{
    /* Load 8-lane initial state */
    ...
    /* Load 8-lane message words (big-endian) */
    ...
    /* Accumulate back to each lane's state */
